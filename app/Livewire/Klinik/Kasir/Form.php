...
class Form extends Component
{
    ...
    public function submit()
    {
        ...
        DB::transaction(function () {
            ...
            $detail[] = $tindakan->whereNotNull('dokter_id')->map(function ($q) use ($pembayaran) {
                return [
                    'kode_akun_id' => '23000',
                    'debet' => 0,
            ...
            $detail[] = $tindakan->whereNotNull('perawat_id')->map(function ($q) use ($pembayaran) {
                return [
                    'kode_akun_id' => '24000',
                    'debet' => 0,
            ...
            $detail[] = $tindakan->map(function ($q) use ($pembayaran) {
                return [
                    'kode_akun_id' => $q['tarifTindakan']->kode_akun_id,
                    'debet' => 0,
                    'kredit' => ($q['biaya'] - $this->diskon - $q['biaya_alat_barang'] - ($q['dokter_id'] ? $q['biaya_jasa_dokter'] : 0) - ($q['perawat_id'] ? $q['biaya_jasa_perawat'] : 0)) * $q['qty'],
                ];
            })->all();
            ...
            //Aset Biaya Alat & Barang
            $detail[] = collect($this->bahan)->map(function ($q) use ($pembayaran) {
                return [
            ...
            $detail[] = collect($this->alat)->map(function ($q) use ($pembayaran) {
                return [
                    'kode_akun_id' => $q['kode_akun_id'],
                    'debet' => 0,
                    'kredit' => $q['biaya'] * $q['qty'],
            ...
            foreach ($this->resep as $resep) {
                ...
                if (count($barangMap)) {
                    BarangClass::stokKeluar($barangMap, $pembayaran->id);
                    //Pendapatan Penjualan Barang
                    $detail[] = collect($barangMap)->map(function ($q) use ($pembayaran) {
                        return [
                            'kode_akun_id' => $q['kode_akun_id'],
                            'debet' => 0,
